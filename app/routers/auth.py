...
from app.schemas.user import Token, UserSignup, UserLogin
...
async def signup(user: UserSignup, db: Session = Depends(get_db)):   # read about sessions
    ...
    access_token = create_access_token(data = {"sub" : user.user_id}, expires_delta = accesss_token_expires)
...
async def login(user: UserLogin, db: Session = Depends(get_db)):
    ...
    if not db_user or not verify_password(plain_password= user.password, hashed_password=db_user.hashed_password):
        raise HTTPException(
